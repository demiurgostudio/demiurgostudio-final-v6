...
window.addEventListener("mousemove", (e) => {
  ...
  [deviceBodyS1, deviceBodyS3].forEach((el) => {
    if (el) gsap.to(el, { rotateY: baseRotY + xNorm * 10, rotateX: baseRotX - yNorm * 8, duration: 0.8, ease: "power2.out" });
  });
  [floorShadowS1, floorShadowS3].forEach((el) => {
    if (el) gsap.to(el, { x: xNorm * 15, y: yNorm * 5, duration: 0.8, ease: "power2.out" });
  });
});
...
function startS1Animation() {
  ...
  tlS1
    ...
    .to(fakeCursorS1, { opacity: 1, x: 155, y: 195, duration: 0.8, ease: "power3.out" })
...
function changeMode(index) {
  ...
  if (frame) {
    gsap.to(frame, { width: targetWidth, height: targetHeight, borderRadius: radius, duration: 1.0, ease: "power2.inOut" });
  }
...
function startS3Animation() {
  ...
  gsap.set(whatsappScreen, { opacity: 0 });
  gsap.set(msgs, { scale: 0, opacity: 0 });
  gsap.set(typing, { scale: 0, opacity: 0 });

  tlS3
    .to(fakeCursorS3, { opacity: 1, x: 235, y: 480, duration: 1.3, ease: "power3.inOut" })
    ...
    .to(msgs[0], { scale: 1, opacity: 1, duration: 0.35, ease: "back.out(1.5)" })
    .to(typing, { scale: 1, opacity: 1, duration: 0.2, delay: 0.2 })
    .to(typing, { scale: 0, opacity: 0, duration: 0.2, delay: 0.6 })
    .to(msgs[1], { scale: 1, opacity: 1, duration: 0.35, ease: "back.out(1.5)" })
    .to(msgs[2], { scale: 1, opacity: 1, duration: 0.35, ease: "back.out(1.5)", delay: 0.4 })
    .to(typing, { scale: 1, opacity: 1, duration: 0.2, delay: 0.2 })
    .to(typing, { scale: 0, opacity: 0, duration: 0.2, delay: 0.6 })
    .to(msgs[3], { scale: 1, opacity: 1, duration: 0.35, ease: "back.out(1.5)" })
    .to({}, { duration: 3.5 })
